// import { FilePenLineIcon, LoaderCircleIcon, LoaderIcon, PencilIcon, Plus, PlusIcon, TrashIcon, UploadCloudIcon, XIcon } from 'lucide-react'
...
//         <p className='text-2xl font-medium mb-6 bg-linear-to-r from-slate-200 to-slate-700 bg-clip-text text-transparent sm:hidden'>Welcome Elly</p>
...
//           </button>
//         </div>
//           <hr  className='border-slate-300 my-6 sm:max-[305px]'/>
...
//                     <input type="text" placeholder='Enter Resume Title' className='w-full py-2 px-4 mb-2 focus:border-green-600 ring-green-600' required />
...
//                     <input onChange={(e)=>{setTitle(e.target.value)}} value={title} type="text" placeholder='Enter Resume Title' className='w-full py-2 px-4 mb-2 focus:border-green-600 ring-green-600' required />
//                     <div>
//                       <label htmlFor="resume-input" className='block text-sm text-slate-700'>
//                         Select Resume File
//                         <div className='flex items-center justify-center  flex-col gap-2 border group text-slate-400 border-slate-400 border-dashed rounded-md p-4 py-10 my-4 hover:border-green-500 hover:text-green-700 cursor-pointer transition-colors'>
//                           {
//                             resume ? (
...
//                                 <p className='text-xs'>Supported formats: .pdf</p>
//                               </>
//                             )
//                           }
//                         </div>
...
//                       {isLoading ? 'UpisLoading...': '  Upload Resume'}
//                     </button>
...
import { FilePenLineIcon, LoaderCircleIcon, LogOutIcon, PencilIcon, PlusIcon, TrashIcon, UploadCloudIcon, XIcon } from 'lucide-react'
import React, { useEffect, useState } from 'react'
import {dummyResumeData} from '../assets/assets'
import { useNavigate } from 'react-router-dom'
import {useSelector} from 'react-redux'
...
const Dashboard = () => {
  ...
  // Example: Logout handler (you can add this to your logout button)
  const handleLogout = () => {
    setConfirmDialog({
      isOpen: true,
      title: 'Logout',
          ...
          {allResumes.map((resume, index) => {
            ...
            return(
              ...
              style={{background:`linear-gradient(135deg, ${baseColor}10, ${baseColor}40)`,borderColor:baseColor+ '40'}}>
                ...
                <div onClick={e => e.stopPropagation()} className='absolute top-1 right-1 group-hover:flex items-center hidden'>
                    <TrashIcon onClick={() => handleDeleteResume(resume._id, resume.title)} className='size-7 p-1.5 hover:bg-white/50 rounded text-slate-700 transition-colors'/>
                    <PencilIcon onClick={() => {setEditResumeId(resume._id); setTitle(resume.title)}} className='size-7 p-1.5 hover:bg-white/50 rounded text-slate-700 transition-colors'/>
                </div>
              </button>
            )
          ...
          <form onSubmit={createResume} onClick={() => {setShowCreateResume(false)}} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
            <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
              ...
              <input 
                ...
                className='w-full py-2 px-4 mb-2 focus:border-green-600 ring-green-600' 
                required 
              />
              <button className='w-full py-2 bg-green-600 text-white rounded hover:bg-green-700 transition-colors'>
                Create Resume
        ...
        {showUploadResume && (
          <form onSubmit={uploadResume} onClick={() => {setShowUploadResume(false)}} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
            <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
              ...
              <button disabled={isLoading} className='w-full py-2 bg-green-600 text-white rounded hover:bg-green-700 transition-colors flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed'>
